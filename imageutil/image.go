...
func Resize(src image.Image, newWidth, newHeight int) image.Image {
	...
	newWidth, newHeight = normalizeResizeTargetSize(srcWidth, srcHeight, newWidth, newHeight)
	if newWidth == 0 || newHeight == 0 {
	...
	case *image.RGBA:
		return resizeRGBA(typed, newWidth, newHeight)
	case *image.Gray:
		return resizeGray(typed, newWidth, newHeight)
	case *image.Gray16:
		return resizeGray16(typed, newWidth, newHeight)
	case *image.YCbCr:
		return resizeYCbCr(typed, newWidth, newHeight)
	default:
		return resizeGeneric(src, newWidth, newHeight)
	}
}
...
func NewRectKernel(width, height int) StructuringElement {
	...
	kernel := make([][]bool, height)
	for y := 0; y < height; y++ {
		kernel[y] = make([]bool, width)
		for x := 0; x < width; x++ {
			kernel[y][x] = true // 矩形核，所有点都参与
		}
	}
...
func NewCrossKernel(size int) StructuringElement {
	...
	kernel := make([][]bool, size)
	anchorPt := size / 2

	for y := 0; y < size; y++ {
		kernel[y] = make([]bool, size)
		for x := 0; x < size; x++ {
			// 只有中心行或中心列为 true
			if x == anchorPt || y == anchorPt {
				kernel[y][x] = true
			} else {
				kernel[y][x] = false
			}
